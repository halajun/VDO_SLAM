...
int SparseOptimizerIncremental::optimize(int iterations, bool online)
{
  ...
  if (!online || batchStep)
  {
    ...
    if (slamDimension == 3)
    {
      ...
      // }
      assert(false && "Sparse Incremental Not implemented for SE2 Vertices");
    }
...
bool SparseOptimizerIncremental::updateInitialization(HyperGraph::VertexSet& vset, HyperGraph::EdgeSet& eset)
{
  ...
  sort(backupIdx, backupIdx + _touchedVertices.size());  // sort according to the hessianIndex which is the same order
                                                         // as used later by the optimizer
  for (int i = 0; i < idx; ++i)
  {
...
bool SparseOptimizerIncremental::computeCholeskyUpdate()
{
  ...
  // writeCCSMatrix("updatesparse.txt", _cholmodSparse->nrow, _cholmodSparse->ncol, (int*)_cholmodSparse->p,
...
static OptimizationAlgorithm* createSolver(const std::string& solverName)
{
  ...
  OptimizationAlgorithmGaussNewton* gaussNewton = new OptimizationAlgorithmGaussNewton(s.get());
  return gaussNewton;
}
...
bool SparseOptimizerIncremental::initSolver(int dimension, int batchEveryN)
{
  ...
  if (dimension == 3)
  {
    ...
    BlockSolver<BlockSolverTraits<3, 2>>* bs =
        dynamic_cast<BlockSolver<BlockSolverTraits<3, 2>>*>(gaussNewton->solver());
  ...
  else
  {
    ...
    BlockSolver<BlockSolverTraits<6, 3>>* bs =
        dynamic_cast<BlockSolver<BlockSolverTraits<6, 3>>*>(gaussNewton->solver());
    assert(bs && "Unable to get internal block solver");
    LinearSolverCholmodOnline<Matrix<double, 6, 6>>* s =
        dynamic_cast<LinearSolverCholmodOnline<Matrix<double, 6, 6>>*>(bs->linearSolver());
