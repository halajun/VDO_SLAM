...
public:
  ...
  bool solve(const SparseBlockMatrix<MatrixType>& A, double* x, double* b)
  {
    ...
    {
      std::cerr << "solve(): Cholesky failure, NOT writing (modified) debug.txt (Hessian loadable by Octave)"
                << std::endl;
  ...
  int choleskyUpdate(cholmod_sparse* update)
  {
    ...
    if (_cholmodCommon.status == CHOLMOD_NOT_POSDEF)
    {
      std::cerr << "Cholesky failure, NOT (modified) writing debug.txt (Hessian loadable by Octave)" << std::endl;
      // writeCCSMatrix("debug.txt", _cholmodSparse->nrow, _cholmodSparse->ncol, (int*)_cholmodSparse->p,
      // (int*)_cholmodSparse->i, (double*)_cholmodSparse->x, true);
      return 0;
    }
...
public:
  void computeSymbolicDecomposition(const SparseBlockMatrix<MatrixType>& A)
  {
    ...
    int amdStatus = camd_order(_matrixStructure.n, _matrixStructure.Ap, _matrixStructure.Aii, _blockPermutation.data(),
