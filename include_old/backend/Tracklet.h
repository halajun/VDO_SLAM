...
public:
  ...
  Tracklet(int tracklet_id_) : tracklet_id(tracklet_id_)
  {
  }
  ...
  void update(const std::vector<std::pair<int, int>>& fontend_tracks_, int tracklet_id_)
  {
    ...
    for (size_t i = observations.size(); i < fontend_tracks_.size(); i++)
    {
      ...
      observations.push_back(
          std::make_shared<Observation<T>>(track.first, track.second, tracklet_position, tracklet_id));
    }
...
struct FrameFeaturePair
{
  ...
  FrameFeaturePair(FrameId frame_id_, FeatureId feature_id_) : frame_id(frame_id_), feature_id(feature_id_)
  {
  }

  bool operator==(const FrameFeaturePair& other) const
  {
    return (frame_id == other.frame_id && feature_id == other.feature_id);
  }
};
...
struct FrameFeatureHash
{
  ...
  {
    return static_cast<unsigned int>(index.frame_id * sl + index.feature_id * sl2);
  }
};
...
struct FrameFeatureHashMapType
{
  ...
  typedef std::unordered_map<FrameFeaturePair, ValueType, FrameFeatureHash> type;
};
