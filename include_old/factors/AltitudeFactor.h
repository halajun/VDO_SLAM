...
public:
  AltitudeFactor(gtsam::Key poseKey, gtsam::Key pointKey, double measured, const gtsam::SharedNoiseModel& model)
    : gtsam::NoiseModelFactor2<gtsam::Pose3, gtsam::Point3>(model, poseKey, pointKey), measured_(measured)
  {
  }

  gtsam::Vector evaluateError(const gtsam::Pose3& pose, const gtsam::Point3& point, boost::optional<gtsam::Matrix&> H1,
