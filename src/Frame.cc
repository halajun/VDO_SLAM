...
void Frame::processStaticFeatures(double depth_background_thresh) {
    ...
    for (int i = 0; i < keypoints.size(); ++i)
    {
        ...
        if (images.depth.at<double>(y,x) > depth_background_thresh || images.depth.at<double>(y,x)<=0)  // new added in Aug 21 2019
            continue;
        ...
        if(flow_xe!=0 && flow_ye!=0)
        {
            if(x+flow_xe < ref_image.cols && y+flow_ye < ref_image.rows && x < ref_image.cols && y < ref_image.rows)
            {
                ...
                feature.predicted_keypoint =cv::KeyPoint(keypoints[i].pt.x+flow_xe,keypoints[i].pt.y+flow_ye,0,0,0,keypoints[i].octave,-1);
