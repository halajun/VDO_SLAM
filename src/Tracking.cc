...
void Tracking::updateFromBackend(const BackendOutput& backend_output)
{
  ...
  previous_frame->pose_ = backend_output.estimated_pose_;
}
...
bool Tracking::solveInitalCamModel(Frame::Ptr previous_frame_, Frame::Ptr current_frame_)
{
  ...
  // std::vector<size_t> tracklet_ids;  // feature tracklets from the current frame so we can assign values as inliers
  // or
  //                                    // outliers
  ...
  //   if (std::find(pnp_tracklet_inliers.begin(), pnp_tracklet_inliers.end(), feature_pair.first) !=
  //       pnp_tracklet_inliers.end())
  //   {
  //     // it is an inlier
  ...
  // current_frame_->pose_ = pose;
  // LOG(INFO) << current_frame_->pose_;
  // return true;
}
