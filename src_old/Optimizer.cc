...
void Optimizer::PartialBatchOptimizationGTSAM(Map* pMap, const cv::Mat Calib_K, const int WINDOW_SIZE)
{
  ...
  {
    // filter the tracklets via threshold
    if (DynTracks[i].size() < 3)
    {  // 3 the length of track on objects.
      continue;
    }
    valid_dyn++;
    // label them
  ...
  for (int i = StaticStartFrame; i < N; ++i)
  {
    ...
    if (i != StaticStartFrame)
    {
      ...
      auto edge_information =
          gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << gtsam::Vector6::Constant(sigma2_cam)).finished());

      graph.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(PreFrameID, CurFrameID, rigid_motion, edge_information);
    ...
    for (int j = 0; j < vnFeaLabSta[i].size(); ++j)
    {
      ...
      if (PositionID == 0)
      {
        ...
        graph.emplace_shared<gtsam::GenericProjectionFactor<gtsam::Pose3, gtsam::Point3, gtsam::Cal3_S2>>(
            projection_measurement, camera_projection_noise, CurFrameID, count_unique_id, K);
      ...
      else
      {
        ...
        const int TrLength = StaTracks[TrackID].size();
        const int FeaMakTmp =
            vnFeaMakSta[StaTracks[TrackID][PositionID - 1].first][StaTracks[TrackID][PositionID - 1].second];
        ...
        gtsam::Point2 projection_measurement(kp.pt.x, kp.pt.y);
        graph.emplace_shared<gtsam::GenericProjectionFactor<gtsam::Pose3, gtsam::Point3, gtsam::Cal3_S2>>(
            projection_measurement, camera_projection_noise, CurFrameID, FeaMakTmp, K);
  ...
  for (int i = StaticStartFrame; i < N; ++i)
  {
    ...
    std::pair<double, double> errors_original =
        utils::computeRotationAndTranslationErrors(original_camera_pose, gt_camera_pose);

    r_sum_original += errors_original.first;
    t_sum_original += errors_original.second;

    std::pair<double, double> errors_refined =
        utils::computeRotationAndTranslationErrors(refined_camera_pose, gt_camera_pose);
      ...
      else
      {
        ...
        for (int j = 1; j < pMap->vmRigidMotion[i - 1].size(); ++j)
        {
          ...
          if (SMOOTH_CONSTRAINT && i > N - WINDOW_SIZE + 2)
          {
            ...
            for (int k = 0; k < pMap->vnRMLabel[i - 2].size(); ++k)
            {
              ...
              {
                // cout << "what is in the label: " << pMap->vnRMLabel[i-2][k] << " " << pMap->vnRMLabel[i-1][j] << " "
                // << VertexID[i-2][k] << endl;
                TraceID = k;
                break;
              }
        ...
        for (int j = 0; j < vnFeaLabDyn[i].size(); j++)
        {
          ...
          else
          {
            ...
            const int TrLength = DynTracks[TrackID].size();
            const int FeaMakTmp =
                vnFeaMakDyn[DynTracks[TrackID][PositionID - 1].first][DynTracks[TrackID][PositionID - 1].second];
...
cv::Mat Optimizer::PoseOptimizationObjMot(Frame* pCurFrame, Frame* pLastFrame, const vector<int>& ObjId,
...
cv::Mat Optimizer::PoseOptimizationFlow2(Frame* pCurFrame, Frame* pLastFrame, const vector<int>& ObjId,
                                         std::vector<int>& InlierID)
{
...
cv::Mat Optimizer::Get3DinWorld(const cv::KeyPoint& Feats2d, const float& Dpts, const cv::Mat& Calib_K,
