  ...
  , objectMotionNoiseModel(nullptr)
{
  ...
  // init plots
  Plotter::initPlot(
      (PlotInfo){ .title = "Error before optimization", .x_label = "# Frames", .y_label = "Chi-squared error" });

  Plotter::initPlot(
      (PlotInfo){ .title = "Error after optimization", .x_label = "# Frames", .y_label = "Chi-squared error" });

  Plotter::initPlot(
      (PlotInfo){ .title = "Time for each update step of iSAM2", .x_label = "# Frames", .y_label = "Time (s)" });

  Plotter::initPlot(
      (PlotInfo){ .title = "Number of factors in the iSAM2 factor graph", .x_label = "# Frames", .y_label = "-" });

  Plotter::initPlot((PlotInfo){ .title = "Number of variables in the system", .x_label = "# Frames", .y_label = "-" });

  Plotter::initPlot((PlotInfo){ .title = "# of Cliques", .x_label = "# Frames", .y_label = "-" });

  Plotter::initPlot((PlotInfo){ .title = "Max Clique Size", .x_label = "# Frames", .y_label = "-" });
...
void VdoSlamBackend::process(bool run_as_incremental)
{
  ...
  if (current_frame > 0)
  {
    ...
    graph.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>((pre_camera_pose_vertex), (curr_camera_pose_vertex),
                                                             rigid_motion, odometryNoiseModel);
    LOG(INFO) << "Added motion constraint factor between id: " << (pre_camera_pose_vertex) << " "
  ...
  for (size_t point_id = 0; point_id < map->vpFeatSta[current_frame].size(); point_id++)
  {
    if (static_tracklets.exists(current_frame, point_id))
    {
      ...
      if (tracklet.isWellTracked())
      {
        ...
        // LOG(INFO) << tracklet.TrackletId() << " is well tracked " << obs_to_add.size() << " and is new: "<<
        // tracklet.isNew();
        ...
        for (StaticTrackletManager::Observation obs : obs_to_add)
        {
          ...
          if (position_id == 0)
          {
            ...
            cv::Mat Xc =
                Optimizer::Get3DinCamera(map->vpFeatSta[frame_id][feature_id], map->vfDepSta[frame_id][feature_id], K);
        ...
        if (tracklet.isWellTracked())
        {
          ...
          for (DynamicTrackletManager::Observation obs : obs_to_add)
          {
            ...
            if (position_id == 0)
            {
              ...
              cv::Mat Xc = Optimizer::Get3DinCamera(map->vpFeatDyn[frame_id][feature_id],
                                                    map->vfDepDyn[frame_id][feature_id], K);
              gtsam::Point3 X_c_point = utils::cvMatToGtsamPoint3(Xc);
            ...
            else
            {
              ...
              //         CHECK(obj_position_id != -1) << "Frame " << previous_frame << " i " << i << " " <<
              //         obj_position_id; break;
              //     }
              // }
              ...
              {
                LOG(INFO) << "Object motion at f: " << frame_id << " motion index " << object_motion_index
                          << " seen first - " << count_unique_id;

                // gtsam::Pose3 object_motion = gtsam::Pose3::identity();
                // gtsam::Pose3 object_motion_camera =
                // utils::cvMatToGtsamPose3(map->vmRigidMotionCamera[previous_frame][object_motion_index]); gtsam::Key
                // previous_pose_key = unique_vertices[previous_frame][0]; gtsam::Pose3 previous_camera_pose =
                // isam->calculateEstimate<gtsam::Pose3>(previous_pose_key); gtsam::Pose3 object_motion = camera_pose *
                // object_motion_camera * camera_pose.inverse();
                gtsam::Pose3 object_motion =
                    utils::cvMatToGtsamPose3(map->vmRigidMotion[previous_frame][object_motion_index]);
                ...
                if (previous_frame > 1)
                {
                  ...
                  if (trace_id != -1)
                  {
                    ...
                    if (state_.exists(previous_motion_key))
                    {
                      ...
                      gtsam::Pose3 object_motion_smoother = gtsam::Pose3::identity();
                      graph.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(
                          previous_motion_key, count_unique_id, object_motion_smoother, objectMotionSmootherNoiseModel);
                    }
                  }
                }
              ...
              // LOG(INFO) << "motion factor - lmk " << dynamic_lmk_key << " prev " << previous_key << " motion key " <<
              // object_motion_key << " frame id" << frame_id << " previous frame id" << previous_frame;
            }
          }
          tracklet.markAsAdded(obs_to_add);
        }
...
const size_t VdoSlamBackend::getMapSize() const
{
...
void VdoSlamBackend::addToKeyVertexMapping(const gtsam::Key& key, FrameId curr_frame, FeatureId feature_id,
...
void VdoSlamBackend::addMotionToGraph(const gtsam::Pose3& motion, gtsam::Key key, FrameId curr_frame,
                                      FeatureId object_id)
{
...
void VdoSlamBackend::addLandmarkToGraph(const gtsam::Point3& landmark, gtsam::Key key, FrameId curr_frame,
...
void VdoSlamBackend::addDynamicLandmarkToGraph(const gtsam::Point3& landmark, gtsam::Key key, FrameId curr_frame,
                                               FeatureId feature_id)
{
  all_values.insert(key, landmark);
...
void VdoSlamBackend::addDynamicPoint3DFactor(const gtsam::Point3& measurement, gtsam::Key pose_key,
                                             gtsam::Key landmark_key)
{
  // CHECK(new_dynamic_lmks.exists(landmark_key) || isam->valueExists(landmark_key)) << "Dyanmic point landmark must be
  // added"; observed_dyn_landmarks[landmark_key].push_back(
  ...
  graph.emplace_shared<Point3DFactor>(pose_key, landmark_key, measurement, dynamicPoint3DNoiseModel);
}
...
{
  graph.emplace_shared<LandmarkMotionTernaryFactor>(previous_point_key, current_point_key, motion_key, measurement,
...
void VdoSlamBackend::optimize()
{
  ...
  if (current_frame > 0)
  {
    ...
    try
    {
      ...
      // gtsam::NonlinearFactorGraph graph_ = gtsam::NonlinearFactorGraph(isam->getFactors());  // clone, expensive but
      // safer!
      gtsam::NonlinearFactorGraph graph_ =
          gtsam::NonlinearFactorGraph(isam->getFactorsUnsafe());  // clone, expensive but safer!
      if (state_.size() > 0)
      {
        ...
        BOOST_FOREACH (const gtsam::Values::ConstKeyValuePair& key_value, all_values)
        {
      ...
      //     LOG(INFO) << "Last frame with dynamic points was frame " << last_update_dynamic_points << " vs. now " <<
      //     current_frame; gtsam::noiseModel::Gaussian::shared_ptr updatedPoseNoise =
      //     gtsam::noiseModel::Gaussian::Covariance(isam->marginalCovariance(curr_camera_pose_vertex));
      // }
