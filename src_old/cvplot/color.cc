...
Color Color::alpha(uint8_t alpha) const
{
...
{
  return Color(pow(r / 255.f, 1 / gamma) * 255, pow(g / 255.f, 1 / gamma) * 255, pow(b / 255.f, 1 / gamma) * 255, a);
}

Color Color::gray(uint8_t v)
{
  return Color(v, v, v);
}

Color Color::index(uint8_t index, uint8_t density, float avoid, float range)
{  // avoid greens by default
  if (avoid > 0)
  {
...
{
  return Color((std::cos(hue * 1.047f) + 1) * 127.9f, (std::cos((hue - 2) * 1.047f) + 1) * 127.9f,
