...
void Series::ensureDimsDepth(int dims, int depth)
{
  if (dims_ != dims)
  {
    if (dims_ != 0)
    {
      std::cerr << "incorrect dims (input dimensions), was " << dims_ << " now " << dims << std::endl;
    }
    dims_ = dims;
  }
  if (depth_ != depth)
  {
    if (depth_ != 0)
    {
      std::cerr << "incorrect depth (output dimensions), was " << depth_ << " now " << depth << std::endl;
    }
    depth_ = depth;
  }
}
...
{
  return set(std::vector<std::pair<float, Point2>>({ { key, { value_a, value_b } } }));
}

Series& Series::set(float key, float value_a, float value_b, float value_c)
{
  return set(std::vector<std::pair<float, Point3>>({ { key, { value_a, value_b, value_c } } }));
}
...
void Series::bounds(float& x_min, float& x_max, float& y_min, float& y_max, int& n_max, int& p_max) const
{
  for (const auto& e : entries_)
  {
    auto xe = e, xd = dims_, ye = e + dims_, yd = depth_ - (dynamic_color_ ? 1 : 0);
    if (type_ == Circle)
    {
      yd = 1;
    }
...
void Series::draw(void* b, float x_min, float x_max, float y_min, float y_max, float xs, float xd, float ys, float yd,
                  float x_axis, float y_axis, int unit, float offset) const
{
  if (dims_ == 0 || depth_ == 0)
  {
    return;
  ...
  switch (type_)
  {
    ...
    case RangeLine:
    {
      if (type_ == FillLine)
      {
        ...
        for (const auto& e : entries_)
        {
          ...
          if (has_last)
          {
            ...
            cv::fillConvexPoly(trans.with(color_.a / 2), points, 4, color, cv::LINE_AA);
          }
          else
          {
            has_last = true;
      ...
      else if (type_ == RangeLine)
      {
        ...
        for (const auto& e : entries_)
        {
          auto x = data_[e], y1 = data_[e + dims_ + 1], y2 = data_[e + dims_ + 2];
          if (dynamic_color_)
          {
            color = color2scalar(Color::cos(data_[e + dims_ + 1]));
          }
      ...
      for (const auto& e : entries_)
      {
        ...
        if (has_last)
        {
          if (type_ == DotLine || type_ == Line || type_ == FillLine || type_ == RangeLine)
          {
            cv::line(trans.with(color_), { (int)(last_x * xs + xd), (int)(last_y * ys + yd) }, point, color, 1,
                     cv::LINE_AA);
          }
        }
        else
      ...
      for (const auto& e : entries_)
      {
        ...
        if (type_ == Histogram)
        {
          cv::rectangle(trans.with(color_), { (int)(x * xs + xd) - u + o, (int)(y_axis * ys + yd) },
                        { (int)(x * xs + xd) + u + o, (int)(y * ys + yd) }, color, -1, cv::LINE_AA);
        }
        else if (type_ == Vistogram)
        {
          cv::rectangle(trans.with(color_), { (int)(x_axis * xs + xd), (int)(x * ys + yd) - u + o },
                        { (int)(y * xs + xd), (int)(x * ys + yd) + u + o }, color, -1, cv::LINE_AA);
        }
      }
    }
    break;
    ...
    case Vertical:
    {
      for (const auto& e : entries_)
      {
        ...
        if (type_ == Horizontal)
        {
          cv::line(trans.with(color_), { (int)(x_min * xs + xd), (int)(y * ys + yd) },
                   { (int)(x_max * xs + xd), (int)(y * ys + yd) }, color, 1, cv::LINE_AA);
        }
        else if (type_ == Vertical)
        {
          cv::line(trans.with(color_), { (int)(y * xs + xd), (int)(y_min * ys + yd) },
...
void Figure::draw(void* b, float x_min, float x_max, float y_min, float y_max, int n_max, int p_max) const
{
  auto& buffer = *(cv::Mat*)b;
  Trans trans(b);

  // draw background and sub axis square
  cv::rectangle(trans.with(background_color_), { 0, 0 }, { buffer.cols, buffer.rows }, color2scalar(background_color_),
                -1, cv::LINE_AA);
  cv::rectangle(trans.with(sub_axis_color_), { border_size_, border_size_ },
                { buffer.cols - border_size_, buffer.rows - border_size_ }, color2scalar(sub_axis_color_), 1,
                cv::LINE_AA);
  ...
  auto x_grid = (x_max != x_min ? value2snap((x_max - x_min) / floor(w_plot / grid_size_)) : 1);
  auto y_grid = (y_max != x_min ? value2snap((y_max - y_min) / floor(h_plot / grid_size_)) : 1);

  // calc affine transform value space to plot space
  auto xs = (x_max != x_min ? (buffer.cols - 2 * border_size_) / (x_max - x_min) : 1.f);
  auto xd = border_size_ - x_min * xs;
  auto ys = (y_max != y_min ? (buffer.rows - 2 * border_size_) / (y_min - y_max) : 1.f);
  auto yd = buffer.rows - y_min * ys - border_size_;

  // safe unit for showing points
  auto unit = std::max(1, ((int)std::min(buffer.cols, buffer.rows) - 2 * border_size_) / n_max / 10);

  // draw sub axis
  for (auto x = ceil(x_min / x_grid) * x_grid; x <= x_max; x += x_grid)
  {
    cv::line(trans.with(sub_axis_color_), { (int)(x * xs + xd), border_size_ },
             { (int)(x * xs + xd), buffer.rows - border_size_ }, color2scalar(sub_axis_color_), 1, cv::LINE_AA);
  }
  for (auto y = ceil(y_min / y_grid) * y_grid; y <= y_max; y += y_grid)
  {
    cv::line(trans.with(sub_axis_color_), { border_size_, (int)(y * ys + yd) },
             { buffer.cols - border_size_, (int)(y * ys + yd) }, color2scalar(sub_axis_color_), 1, cv::LINE_AA);
  }
  ...
  for (auto x = std::ceil(x_min / x_grid) * x_grid; x <= x_max; x += x_grid)
  {
    ...
    int baseline;
    cv::Size size = getTextSize(out.str(), cv::FONT_HERSHEY_SIMPLEX, 0.3f, 1.f, &baseline);
    cv::Point org(x * xs + xd - size.width / 2, buffer.rows - border_size_ + 5 + size.height);
    cv::putText(trans.with(text_color_), out.str().c_str(), org, cv::FONT_HERSHEY_SIMPLEX, 0.3f,
                color2scalar(text_color_), 1.f);
  }
  ...
  cv::line(trans.with(text_color_), { border_size_, (int)(y_axis * ys + yd) },
           { buffer.cols - border_size_, (int)(y_axis * ys + yd) }, color2scalar(text_color_), 1, cv::LINE_AA);
  cv::line(trans.with(axis_color_), { (int)(x_axis * xs + xd), border_size_ },
           { (int)(x_axis * xs + xd), buffer.rows - border_size_ }, color2scalar(axis_color_), 1, cv::LINE_AA);
  ...
  for (auto s = series_.rbegin(); s != series_.rend(); ++s)
  {
    ...
    s->draw(&trans.with(s->color()), x_min, x_max, y_min, y_max, xs, xd, ys, yd, x_axis, y_axis, unit,
  ...
  for (const auto& s : series_)
  {
    ...
    cv::Point org(buffer.cols - border_size_ - size.width - 17, border_size_ + 15 * index + 15);
    auto shadow = true;
    cv::putText(trans.with(background_color_), name.c_str(), { org.x + (shadow ? 1 : 0), org.y + (shadow ? 1 : 0) },
                cv::FONT_HERSHEY_SIMPLEX, 0.4f, color2scalar(background_color_), (shadow ? 1.f : 2.f));
    ...
    cv::putText(trans.with(text_color_), name.c_str(), org, cv::FONT_HERSHEY_SIMPLEX, 0.4f, color2scalar(text_color_),
                1.f);
    s.dot(&trans.with(s.color()), buffer.cols - border_size_ - 10, org.y - 3, 3);
    index++;
  }
}
...
Figure& figure(const std::string& name)
{
  if (shared_figures_.count(name) == 0)
  {
    ...
    shared_figures_.insert(std::map<std::string, Figure>::value_type(name, Figure(view)));
  }
