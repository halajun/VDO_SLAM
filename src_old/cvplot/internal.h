...
#define EXPECT_EQ(a__, b__)                                                                                            \
  ...
  {                                                                                                                    \
    if ((a__) != (b__))                                                                                                \
    {                                                                                                                  \
      std::cerr << "Incorrect " << #a__ << " (" << (a__) << "), should equal " << (b__) << std::endl;                  \
      exit(-1);                                                                                                        \
    }                                                                                                                  \
  } while (0)
...
{
  return cv::Scalar(channel2pale(color.b), channel2pale(color.g), channel2pale(color.r));
}

static float value2snap(float value)
{
  return std::max(
      { pow(10, floor(log10(value))), pow(10, floor(log10(value / 2))) * 2, pow(10, floor(log10(value / 5))) * 5 });
}
