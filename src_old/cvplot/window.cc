...
void View::drawRect(Rect rect, Color color)
{
  ...
  cv::rectangle(trans.with(color), { rect_.x + rect.x, rect_.y + rect.y },
                { rect_.x + rect.x + rect.width, rect_.y + rect.y + rect.height }, color2scalar(color), -1);
  window_.dirty();
}
...
void View::drawText(const std::string& text, Offset offset, Color color) const
{
  ...
  Trans trans(window_.buffer());
  cv::putText(trans.with(color), text.c_str(), org, face, scale, color2scalar(color), thickness);
  window_.dirty();
}
...
void View::drawFrame(const std::string& title) const
{
  ...
  cv::rectangle(trans.with(background_color_), { rect_.x, rect_.y },
                { rect_.x + rect_.width - 1, rect_.y + rect_.height - 1 }, color2scalar(background_color_), 1);
  cv::rectangle(trans.with(frame_color_), { rect_.x + 1, rect_.y + 1 },
                { rect_.x + rect_.width - 2, rect_.y + rect_.height - 2 }, color2scalar(frame_color_), 1);
  cv::rectangle(trans.with(frame_color_), { rect_.x + 2, rect_.y + 2 }, { rect_.x + rect_.width - 3, rect_.y + 16 },
                color2scalar(frame_color_), -1);
  int baseline;
  cv::Size size = getTextSize(title.c_str(), cv::FONT_HERSHEY_PLAIN, 1.f, 1.f, &baseline);
  cv::putText(trans.with(text_color_), title.c_str(), { rect_.x + 2 + (rect_.width - size.width) / 2, rect_.y + 14 },
...
void View::drawImage(const void* image, int alpha)
{
  ...
  if (img.cols != rect_.width || img.rows != rect_.height)
  {
    ...
    resized.copyTo(trans.with(alpha)({ rect_.x, rect_.y, rect_.width, rect_.height }));
  }
  else
  {
    img.copyTo(trans.with(alpha)({ rect_.x, rect_.y, rect_.width, rect_.height }));
  }
  window_.dirty();
}
...
{
  Trans trans(window_.buffer());
  cv::rectangle(trans.with(color), { rect_.x, rect_.y }, { rect_.x + rect_.width - 1, rect_.y + rect_.height - 1 },
...
{
  return offset.x >= rect_.x && offset.y >= rect_.y && offset.x < rect_.x + rect_.width &&
         offset.y < rect_.y + rect_.height;
}
...
{
  auto& buffer = *(new cv::Mat(cv::Size(size.width, size.height), CV_8UC3, color2scalar(Gray)));
  if (buffer_ != NULL)
  {
    auto& current = *(cv::Mat*)buffer_;
    if (current.cols > 0 && current.rows > 0 && size.width > 0 && size.height > 0)
    {
      cv::Rect inter(0, 0, std::min(current.cols, size.width), std::min(current.rows, size.height));
...
Window& Window::ensure(Rect rect)
{
  ...
  else
  {
    ...
    {
      size({ std::max(b.cols, rect.x + rect.width), std::max(b.rows, rect.y + rect.height) });
    }
  }
  return *this;
}
...
void Window::flush()
{
  if (dirty_ && buffer_ != NULL)
  {
    ...
    if (b->cols > 0 && b->rows > 0)
    {
      ...
      {
        b->copyTo(mat);
        cv::line(mat, { cursor_.x - 4, cursor_.y + 1 }, { cursor_.x + 6, cursor_.y + 1 }, color2scalar(White), 1);
        cv::line(mat, { cursor_.x + 1, cursor_.y - 4 }, { cursor_.x + 1, cursor_.y + 6 }, color2scalar(White), 1);
        cv::line(mat, { cursor_.x - 5, cursor_.y }, { cursor_.x + 5, cursor_.y }, color2scalar(Black), 1);
        cv::line(mat, { cursor_.x, cursor_.y - 5 }, { cursor_.x, cursor_.y + 5 }, color2scalar(Black), 1);
        b = &mat;
      }
      cv::namedWindow(name_, cv::WINDOW_AUTOSIZE);
...
View& Window::view(const std::string& name, Size size)
{
  ...
  {
    views_.insert(std::map<std::string, View>::value_type(name, View(*this, name, size)));
  }
