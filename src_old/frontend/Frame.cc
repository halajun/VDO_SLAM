...
Frame::Frame(const Frame& frame)
  ...
  ,
  // new added
  mTcw_gt(frame.mTcw_gt)
  , vObjPose_gt(frame.vObjPose_gt)
  , nSemPosi_gt(frame.nSemPosi_gt)
  , vObjLabel(frame.vObjLabel)
  , nModLabel(frame.nModLabel)
  , nSemPosition(frame.nSemPosition)
  , bObjStat(frame.bObjStat)
  , vObjMod(frame.vObjMod)
  , vObjModCamera(frame.vObjModCamera)
  , mvCorres(frame.mvCorres)
  , mvObjCorres(frame.mvObjCorres)
  , mvFlowNext(frame.mvFlowNext)
  , mvObjFlowNext(frame.mvObjFlowNext)
  , mvStat3DPointTmp(frame.mvStat3DPointTmp)
  , mvObj3DPoint(frame.mvObj3DPoint)
  , rgb(frame.rgb.clone())
  ...
  , mThDepthObj(thDepthObj)
{
  ...
  if (UseSampleFea == 0)
  {
    ...
    for (int i = 0; i < mvKeys.size(); ++i)
    {
      ...
      {
        if (mvKeys[i].pt.x + flow_xe < gray.cols && mvKeys[i].pt.y + flow_ye < gray.rows &&
            mvKeys[i].pt.x < gray.cols && mvKeys[i].pt.y < gray.rows)
        {
          mvStatKeysTmp.push_back(mvKeys[i]);
          mvCorres.push_back(
              cv::KeyPoint(mvKeys[i].pt.x + flow_xe, mvKeys[i].pt.y + flow_ye, 0, 0, 0, mvKeys[i].octave, -1));
  ...
  else
  {
    ...
    for (int i = 0; i < mvKeysSamp.size(); ++i)
    {
      ...
      if (flow_xe != 0 && flow_ye != 0)
      {
        if (mvKeysSamp[i].pt.x + flow_xe < gray.cols && mvKeysSamp[i].pt.y + flow_ye < gray.rows &&
            mvKeysSamp[i].pt.x + flow_xe > 0 && mvKeysSamp[i].pt.y + flow_ye > 0)
        {
          mvStatKeysTmp.push_back(mvKeysSamp[i]);
          mvCorres.push_back(cv::KeyPoint(mvKeysSamp[i].pt.x + flow_xe, mvKeysSamp[i].pt.y + flow_ye, 0, 0, 0,
  ...
  while (key_num < N)
  {
    for (int i = 0; i < n_div; ++i)
    {
      for (int j = 0; j < n_div; ++j)
      {
        ...
        // {
        //     float cur_dist = std::sqrt( (KeyinGrid[].pt.x-x)*(KeyinGrid[].pt.x-x) +
        //     (KeyinGrid[].pt.y-y)*(KeyinGrid[].pt.y-y) ); if (cur_dist<min_dist)
        //         min_dist = cur_dist;
        //     if (min_dist<5.0)
        //     {
        //         used = true;
