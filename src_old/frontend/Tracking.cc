...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat& imRGB, cv::Mat& imD, const cv::Mat& imFlow, const cv::Mat& maskSEM,
                                ...
                                const double& timestamp, cv::Mat& imTraj, const int& nImage)
{
  ...
  mCurrentFrame = Frame(imRGB, imDepth, imFlow, maskSEM, timestamp, mpORBextractorLeft, mK, mDistCoef, mbf, mThDepth,
                        mThDepthObj, nUseSampleFea);
  ...
  if (mState != NO_IMAGES_YET)
  {
    ...
    for (int i = 0; i < mCurrentFrame.mvObjKeys.size(); ++i)
    {
      ...
      if (u < (mImGray.cols - 1) && u > 0 && v < (mImGray.rows - 1) && v > 0 && imDepth.at<float>(v, u) < mThDepthObj &&
    ...
    // // show image
    // cv::Mat img_show;
    // cv::drawKeypoints(mImGray, mCurrentFrame.mvObjKeys, img_show, cv::Scalar::all(-1),
    // cv::DrawMatchesFlags::DEFAULT); cv::imshow("Dense Feature Distribution 2", img_show); cv::waitKey(0);
  ...
  if (timestamp != 0 && bFrame2Frame == true)
  {
    ...
    //     if (KeyPoints_tmp[0].pt.x>=(mImGray.cols-1) || KeyPoints_tmp[0].pt.x<=0 ||
    //     KeyPoints_tmp[0].pt.y>=(mImGray.rows-1) || KeyPoints_tmp[0].pt.y<=0)
    //         continue;
    //     if(maskSEM.at<int>(KeyPoints_tmp[0].pt.y,KeyPoints_tmp[0].pt.x)!=0)
    //         continue;
  ...
  if (timestamp != 0 && bFrame2Frame == true && mTestData == KITTI)
  {
    ...
    for (int i = 0; i < mCurrentFrame.vObjBoxID.size(); ++i)
    {
      ...
      cv::putText(mImBGR, output_est, cv::Point(pt1.x, pt1.y - 10), cv::FONT_HERSHEY_DUPLEX, 0.9, CV_RGB(0, 255, 0),
                  2);  // CV_RGB(255,140,0)
  ...
  if (mTestData == KITTI)
  {
    ...
    cv::putText(imTraj, "Camera Trajectory (RED SQUARE)", cv::Point(10, 30), cv::FONT_HERSHEY_COMPLEX, 0.6,
                CV_RGB(255, 255, 255), 1);
    char text[100];
    sprintf(text, "x = %02fm y = %02fm z = %02fm", CamPos.at<float>(0, 3), CamPos.at<float>(1, 3),
            CamPos.at<float>(2, 3));
    cv::putText(imTraj, text, cv::Point(10, 50), cv::FONT_HERSHEY_COMPLEX, 0.6, cv::Scalar::all(255), 1);
    cv::putText(imTraj, "Object Trajectories (COLORED CIRCLES)", cv::Point(10, 70), cv::FONT_HERSHEY_COMPLEX, 0.6,
                CV_RGB(255, 255, 255), 1);
  ...
  {
    PlotMetricError(mpMap->vmCameraPose, mpMap->vmRigidMotion, mpMap->vmObjPosePre, mpMap->vmCameraPose_GT,
...
void Tracking::Track()
{
  ...
  else
  {
    ...
    float t_rpe_cam = std::sqrt(RePoEr_cam.at<float>(0, 3) * RePoEr_cam.at<float>(0, 3) +
    ...
    mCurrentFrame.vObjModCamera.resize(ObjIdNew.size());
    mCurrentFrame.vObjPosePre.resize(ObjIdNew.size());
    mCurrentFrame.vObjMod_gt.resize(ObjIdNew.size());
    mCurrentFrame.vObjModCamera_gt.resize(ObjIdNew.size());
    ...
    for (int i = 0; i < ObjIdNew.size(); ++i)
    {
      ...
      cv::Mat L_c_prev, L_c_curr, L_w_prev, L_w_curr, H_p_c, H_p_c_w, H_p_c_c;
      ...
      for (int k = 0; k < mLastFrame.nSemPosi_gt.size(); ++k)
      {
        if (mLastFrame.nSemPosi_gt[k] == mCurrentFrame.nSemPosition[i])
        {
          ...
          if (mTestData == OMD)
          {
            L_w_prev = mLastFrame.vObjPose_gt[k];
          }
          else if (mTestData == KITTI)
          {
            L_c_prev = mLastFrame.vObjPose_gt[k];
            // cout << "what is L_p: " << endl << L_p << endl;
            L_w_prev = Last_Twc_gt * L_c_prev;
      ...
      for (int k = 0; k < mCurrentFrame.nSemPosi_gt.size(); ++k)
      {
        if (mCurrentFrame.nSemPosi_gt[k] == mCurrentFrame.nSemPosition[i])
        {
          ...
          if (mTestData == OMD)
          {
            L_w_curr = mCurrentFrame.vObjPose_gt[k];
          }
          else if (mTestData == KITTI)
          {
            L_c_curr = mCurrentFrame.vObjPose_gt[k];
            // cout << "what is L_c: " << endl << L_c << endl;
            L_w_curr = Curr_Twc_gt * L_c_curr;
      ...
      if (!bCheckGT1 || !bCheckGT2)
      {
        ...
        mCurrentFrame.vObjModCamera[i] = cv::Mat::eye(4, 4, CV_32F);
        mCurrentFrame.vObjModCamera_gt[i] = cv::Mat::eye(4, 4, CV_32F);
      ...
      // Object pose L in camera frame at t -1 (inverse)
      cv::Mat L_c_prev_inv = Converter::toInvMatrix(L_c_prev);
      // Object pose L in camera frame at t (inverse)
      cv::Mat L_c_inv_curr = Converter::toInvMatrix(L_c_curr);
      // motion H in frame L from t-1 to t
      cv::Mat H_l_prev_curr = L_c_prev_inv * L_c_curr;

      cv::Mat L_w_prev_inv = Converter::toInvMatrix(L_w_prev);
      // motion H in world frame from t-1 to t
      cv::Mat H_w_prev_curr = L_w_prev * H_l_prev_curr * L_w_prev_inv;
      // cv::Mat L_w_p_inv = Converter::toInvMatrix(L_w_p);
      // cv::Mat L_w_c_inv = Converter::toInvMatrix(L_w_c);
      // cv::Mat H_p_c_l = L_w_c*L_w_p_inv;
      // // H_p_c_body = Converter::toInvMatrix(L_p)*L_c; // for new metric (26 Feb 2020).
      // H_p_c_body = L_w_p_inv*L_w_c; // actually world

      // motion H in camera frame from t-1 to t
      cv::Mat H_c_prev_curr = L_c_prev * H_l_prev_curr * L_c_prev_inv;
      mCurrentFrame.vObjModCamera_gt[i] = H_c_prev_curr;
      // cv::Mat H_p_c_world =
      mCurrentFrame.vObjMod_gt[i] = H_w_prev_curr;
      // mCurrentFrame.vObjCentre3D[i] = L_w_p.rowRange(0,3).col(3);
      mCurrentFrame.vObjPosePre[i] = L_w_prev;  // for new metric (26 Feb 2020).
      ...
      // cout << "number of pick points: " << ObjIdTest_in.size() << "/" << ObjIdTest.size() << "/" <<
      // mCurrentFrame.mvObjKeys.size() << endl;
      ...
      else
      {
        mCurrentFrame.vObjMod[i] =
            Optimizer::PoseOptimizationObjMot(&mCurrentFrame, &mLastFrame, ObjIdTest_in, InlierID);
      }
      // mCurrentFrame.vObjMod[i] = Optimizer::PoseOptimizationObjMot(&mCurrentFrame,&mLastFrame,ObjIdTest_in,InlierID);
      e_3_2 = clock();
      t_con = t_con + 1;
      obj_mot_time = obj_mot_time + (double)(e_3_1 - s_3_1) / CLOCKS_PER_SEC * 1000 +
      ...
      sp_gt_v = H_c_prev_curr.rowRange(0, 3).col(3) -
                (cv::Mat::eye(3, 3, CV_32F) - H_c_prev_curr.rowRange(0, 3).colRange(0, 3)) *
                    ObjCentre3D_pre;  // L_w_p.rowRange(0,3).col(3) or ObjCentre3D_pre
      sp_gt_v2 = L_w_prev.rowRange(0, 3).col(3) - L_w_curr.rowRange(0, 3).col(3);
      float sp_gt_norm =
          std::sqrt(sp_gt_v.at<float>(0) * sp_gt_v.at<float>(0) + sp_gt_v.at<float>(1) * sp_gt_v.at<float>(1) +
                    sp_gt_v.at<float>(2) * sp_gt_v.at<float>(2)) *
          36;
      // float sp_gt_norm2 = std::sqrt( sp_gt_v2.at<float>(0)*sp_gt_v2.at<float>(0) +
      // sp_gt_v2.at<float>(1)*sp_gt_v2.at<float>(1) + sp_gt_v2.at<float>(2)*sp_gt_v2.at<float>(2) )*36;
      mCurrentFrame.vObjSpeed_gt[i] = sp_gt_norm;

      // // ***** calculate the estimated object speed *****
      cv::Mat sp_est_v;
      sp_est_v =
          mCurrentFrame.vObjMod[i].rowRange(0, 3).col(3) -
          (cv::Mat::eye(3, 3, CV_32F) - mCurrentFrame.vObjMod[i].rowRange(0, 3).colRange(0, 3)) * ObjCentre3D_pre;
      float sp_est_norm =
          std::sqrt(sp_est_v.at<float>(0) * sp_est_v.at<float>(0) + sp_est_v.at<float>(1) * sp_est_v.at<float>(1) +
      ...
      // RePoEr.at<float>(1,3)*RePoEr.at<float>(1,3) + RePoEr.at<float>(2,3)*RePoEr.at<float>(2,3) ); float trace_rpe =
      // 0; for (int i = 0; i < 3; ++i)
      // {
    ...
    std::vector<cv::Mat> Mot_Tmp, Mot_Tmp_Camera, ObjPose_Tmp;
    ...
    Mot_Tmp_Camera.push_back(CameraMotionTmp);
    ...
    mpMap->vmRigidMotionCamera.push_back(Mot_Tmp_Camera);
    ...
    std::vector<cv::Mat> Mot_Tmp_gt, Mot_Tmp_Camera_gt;
    ...
    Mot_Tmp_Camera_gt.push_back(CameraMotionTmp_gt);
    ...
    for (int i = 0; i < mCurrentFrame.vObjMod_gt.size(); ++i)
    {
      ...
      Mot_Tmp_Camera_gt.push_back(mCurrentFrame.vObjModCamera_gt[i]);
    }
    // (8.3) Save to The Map
    mpMap->vmRigidMotion_GT.push_back(Mot_Tmp_gt);
    mpMap->vmRigidMotionCamera_GT.push_back(Mot_Tmp_Camera_gt);
  ...
  if (run_as_incremental && f_id > 2)
  {
    ...
    LOG(INFO) << "Obj points" << mLastFrame.mvObj3DPoint.size() << " " << mLastFrame.vSemObjLabel.size() << " "
              << mLastFrame.vObjLabel.size();
    for (int i = 0; i < mLastFrame.mvObj3DPoint.size(); ++i)
    {
      // updating static points
      mLastFrame.mvObj3DPoint[i] = mpMap->vp3DPointDyn[last_update_dynamic_points][i].clone();
  ...
  if (f_id == StopFrame)  // bFrame2Frame f_id>=2
  {
    ...
    else
    {
      ...
      GetMetricError(mpMap->vmCameraPose, mpMap->vmRigidMotion, mpMap->vmObjPosePre, mpMap->vmCameraPose_GT,
                     mpMap->vmRigidMotion_GT, mpMap->vbObjStat);
...
void Tracking::Initialization()
{
  ...
  // initialize the 3d points
  {
    ...
    {
      mv3DPointTmp.push_back(
          Optimizer::Get3DinCamera(mCurrentFrame.mvStatKeysTmp[i], mCurrentFrame.mvStatDepthTmp[i], mK));
    }
    ...
    // cout << "see the size 1: " << mCurrentFrame.mvStatKeysTmp.size() << " " << mCurrentFrame.mvSift3DPoint.size() <<
    // endl; cout << "see the size 2: " << mCurrentFrame.mvObjKeys.size() << " " << mCurrentFrame.mvObj3DPoint.size() <<
    // endl;
  }
...
void Tracking::GetSceneFlowObj()
{
  ...
  // Tracking::DrawSparseFlowBirdeye(pts_p3d, pts_vel, Converter::toInvMatrix(mLastFrame.mTcw), viz_props,
  // img_sparse_flow_3d); cv::imshow("SparseFlowBirdeye", img_sparse_flow_3d*255); cv::waitKey(0);
}
...
std::vector<std::vector<int> > Tracking::DynObjTracking()
{
  ...
  for (int i = 0; i < Posi.size(); ++i)
  {
    ...
    for (int j = 0; j < Posi[i].size(); ++j)
    {
      ...
      if (v < shrin_thr_row || v > (mImGray.rows - shrin_thr_row) || u < shrin_thr_col ||
          u > (mImGray.cols - shrin_thr_col))
        count = count + 1;
    }
  ...
  for (int i = 0; i < ObjId.size(); ++i)
  {
    ...
    for (int j = 0; j < ObjId[i].size(); ++j)
    {
      ...
      float sf_norm = std::sqrt(mCurrentFrame.vFlow_3d[ObjId[i][j]].x * mCurrentFrame.vFlow_3d[ObjId[i][j]].x +
    ...
    {
      obj_dis_tres[i] = -1;
      // cout << "object " << sem_posi[i] <<" is too far away or too small! " << obj_center_depth/ObjId[i].size() <<
      // endl; label this object as far away object
      for (int k = 0; k < ObjId[i].size(); ++k)
...
cv::Mat Tracking::GetInitModelCam(const std::vector<int>& MatchId, std::vector<int>& MatchId_sub)
{
  ...
  cv::solvePnPRansac(pre_3d, cur_2d, camera_mat, distCoeffs, Rvec, Tvec, false, iter_num, reprojectionError, confidence,
                     inliers, cv::SOLVEPNP_AP3P);  // AP3P EPNP P3P ITERATIVE DLS
  ...
  // cout << "Inlier Compare: " << "(1)AP3P RANSAC: " << inliers.rows << " (2)Motion Model: " << MM_inlier.size() <<
  // endl;
...
void Tracking::DrawLine(cv::KeyPoint& keys, cv::Point2f& flow, cv::Mat& ref_image, const cv::Scalar& color,
                        int thickness, int line_type, const cv::Point2i& offset)
{
    ...
    for (int j = -radius; j < radius; j++)
    {
      ...
      if (coord_x > 0 && coord_y > 0 && coord_x < ref_image.cols && coord_y < ref_image.rows)
      {
        ref_image.at<cv::Vec3b>(cv::Point(coord_x, coord_y)) =
            (1.0 - alpha) * ref_image.at<cv::Vec3b>(cv::Point(coord_x, coord_y)) + alpha * color;
      }
    }
  }
}
...
void Tracking::DrawSparseFlowBirdeye(const std::vector<Eigen::Vector3d>& pts, const std::vector<Eigen::Vector3d>& vel,
                                     const cv::Mat& camera, const BirdEyeVizProperties& viz_props, cv::Mat& ref_image)
{
  ...
  for (int i = 0; i < pts.size(); i++)
  {
    ...
    if (x_1 > viz_props.birdeye_left_plane_ && x_2 > viz_props.birdeye_left_plane_ &&
        x_1 < viz_props.birdeye_right_plane_ && x_2 < viz_props.birdeye_right_plane_ && z_1 > 0 && z_2 > 0 &&
  ...
  cv::arrowedLine(ref_image, cv::Point(ref_image.cols / 2, offset_y),
                  cv::Point(ref_image.cols / 2 + arrow_len, offset_y), cv::Scalar(1.0, 0, 0), 2);
  cv::arrowedLine(ref_image, cv::Point(ref_image.cols / 2, offset_y),
                  cv::Point(ref_image.cols / 2, offset_y + arrow_len), cv::Scalar(0.0, 1.0, 0), 2);

  // cv::putText(ref_image, "X", cv::Point(ref_image.cols/2+arrow_len+10, offset_y+10), cv::FONT_HERSHEY_COMPLEX_SMALL,
  // 0.8, cv::Scalar(1.0, 0, 0)); cv::putText(ref_image, "Z", cv::Point(ref_image.cols/2+10, offset_y+arrow_len),
  // cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cv::Scalar(0.0, 1.0, 0));
...
cv::Mat Tracking::ObjPoseParsingOX(const std::vector<float>& vObjPose_gt)
{
  ...
  const float angle =
      std::sqrt(vObjPose_gt[5] * vObjPose_gt[5] + vObjPose_gt[6] * vObjPose_gt[6] + vObjPose_gt[7] * vObjPose_gt[7]);
...
std::vector<std::vector<std::pair<int, int> > > Tracking::GetStaticTrack()
{
  ...
  // for (int i = 0; i < N; ++i)
  //     cout << "The length of " << i+2 << " tracklets is found with the amount of " << TrackLength[i] << " ..." <<
  //     endl;
  // cout << endl;
...
std::vector<std::vector<int> > Tracking::GetObjTrackTime(std::vector<std::vector<int> >& ObjTrackLab,
...
void Tracking::RenewFrameInfo(const std::vector<int>& TM_sta)
{
  ...
  for (int i = 0; i < TM_sta.size(); ++i)
  {
    ...
    {
      if (mCurrentFrame.mvStatKeys[TM_sta[i]].pt.x + flow_xe < mImGrayLast.cols &&
          mCurrentFrame.mvStatKeys[TM_sta[i]].pt.y + flow_ye < mImGrayLast.rows &&
          ...
          mCurrentFrame.mvStatKeys[TM_sta[i]].pt.y + flow_ye > 0)
      {
        ...
        mvCorresTmp.push_back(cv::KeyPoint(mCurrentFrame.mvStatKeys[TM_sta[i]].pt.x + flow_xe,
  ...
  while (tot_num < max_num_sta)
  {
    ...
    for (int i = start_id; i < mvKeysSample.size(); i = i + step)
    {
      ...
      {
        float cur_dist = std::sqrt(
            (mvKeysTmpCheck[j].pt.x - mvKeysSample[i].pt.x) * (mvKeysTmpCheck[j].pt.x - mvKeysSample[i].pt.x) +
            (mvKeysTmpCheck[j].pt.y - mvKeysSample[i].pt.y) * (mvKeysTmpCheck[j].pt.y - mvKeysSample[i].pt.y));
      ...
      if (flow_xe != 0 && flow_ye != 0)
      {
            ...
            mvKeysSample[i].pt.x + flow_xe > 0 && mvKeysSample[i].pt.y + flow_ye > 0)
        {
          ...
          mvCorresTmp.push_back(
              cv::KeyPoint(mvKeysSample[i].pt.x + flow_xe, mvKeysSample[i].pt.y + flow_ye, 0, 0, 0, -1));
          mvFlowNextTmp.push_back(cv::Point2f(flow_xe, flow_ye));
  ...
  {
    mv3DPointTmp[i] =
        Optimizer::Get3DinWorld(mvKeysTmp[i], mvDepthTmp[i], mK, Converter::toInvMatrix(mCurrentFrame.mTcw));
  }
  ...
  for (int i = 0; i < ObjSet.size(); ++i)
  {
    ...
    while (tot_num < max_num_obj)
    {
      ...
      for (int j = start_id; j < mvTmpSemObjLabel.size(); j = j + step)
      {
        ...
        {
          float cur_dist = std::sqrt(
              (mvObjKeysTmpCheck[k].pt.x - mvTmpObjKeys[j].pt.x) * (mvObjKeysTmpCheck[k].pt.x - mvTmpObjKeys[j].pt.x) +
              (mvObjKeysTmpCheck[k].pt.y - mvTmpObjKeys[j].pt.y) * (mvObjKeysTmpCheck[k].pt.y - mvTmpObjKeys[j].pt.y));
          if (cur_dist < min_dist)
            min_dist = cur_dist;
          if (min_dist < 1.0)
          {
  ...
  for (int i = 0; i < mvObjKeysTmp.size(); ++i)
    mvObj3DPointTmp[i] =
        Optimizer::Get3DinWorld(mvObjKeysTmp[i], mvObjDepthTmp[i], mK, Converter::toInvMatrix(mCurrentFrame.mTcw));
                              ...
                              const std::vector<std::vector<bool> >& ObjStat)
{
  ...
  for (int i = 1; i < CamPose.size(); ++i)
  {
    ...
    // translation
    float t_ate_cam = std::sqrt(ate_cam.at<float>(0, 3) * ate_cam.at<float>(0, 3) +
  ...
  for (int i = 0; i < RigMot.size(); ++i)
  {
    if (RigMot[i].size() > 1)
    {
      for (int j = 1; j < RigMot[i].size(); ++j)
      {
        ...
        // translation error
        float t_rpe_obj = std::sqrt(rpe_obj.at<float>(0, 3) * rpe_obj.at<float>(0, 3) +
  ...
  for (int i = 0; i < each_obj_count.size(); ++i)
  {
    ...
    if (each_obj_count[i] >= 3)
      cout << endl
           ...
           << " t: " << each_obj_t[i] << " R: " << each_obj_r[i] << " TrackCount: " << each_obj_count[i] << endl;
  }

  cout << "=================================================" << endl;
}

void Tracking::PlotMetricError(const std::vector<cv::Mat>& CamPose, const std::vector<std::vector<cv::Mat> >& RigMot,
                               const std::vector<std::vector<cv::Mat> >& ObjPosePre,
                               const std::vector<cv::Mat>& CamPose_gt,
                               const std::vector<std::vector<cv::Mat> >& RigMot_gt,
                               const std::vector<std::vector<bool> >& ObjStat)
{
  ...
  for (int i = 0; i < RigMot.size(); ++i)
  {
    if (RigMot[i].size() > 1)
    {
      for (int j = 1; j < RigMot[i].size(); ++j)
      {
        ...
        if (bAccumError)
        {
          ObjTraErr[mpMap->vnRMLabel[i][j] - 1].push_back(each_obj_t[mpMap->vnRMLabel[i][j] - 1] /
                                                          each_obj_count[mpMap->vnRMLabel[i][j] - 1]);
          ObjRotErr[mpMap->vnRMLabel[i][j] - 1].push_back(each_obj_r[mpMap->vnRMLabel[i][j] - 1] /
                                                          each_obj_count[mpMap->vnRMLabel[i][j] - 1]);
        }
        else
        {
  ...
  figure1.series("Camera").setValue(CamTraErr).type(cvplot::DotLine).color(cvplot::Red);

  figure2.series("Camera").setValue(CamRotErr).type(cvplot::DotLine).color(cvplot::Red);
...
void Tracking::GetVelocityError(const std::vector<std::vector<cv::Mat> >& RigMot,
                                ...
                                const std::vector<std::vector<bool> >& ObjStat)
{
    ...
    if (RigMot[i].size() > 1 && Velo_gt[i].size() > 1 && RMLab[i].size() > 1)
    {
      ...
      for (int j = 1; j < RigMot[i].size(); ++j)
      {
        ...
        cv::Mat sp_est_v = RigMot[i][j].rowRange(0, 3).col(3) -
                           (cv::Mat::eye(3, 3, CV_32F) - RigMot[i][j].rowRange(0, 3).colRange(0, 3)) * ObjCenter;
        float sp_est_norm =
            std::sqrt(sp_est_v.at<float>(0) * sp_est_v.at<float>(0) + sp_est_v.at<float>(1) * sp_est_v.at<float>(1) +
                      sp_est_v.at<float>(2) * sp_est_v.at<float>(2)) *
            36;
        ...
        if (bRMSError)
        {
          each_obj_est[mpMap->vnRMLabel[i][j] - 1] =
              each_obj_est[mpMap->vnRMLabel[i][j] - 1] + sp_est_norm * sp_est_norm;
          each_obj_gt[mpMap->vnRMLabel[i][j] - 1] =
              each_obj_gt[mpMap->vnRMLabel[i][j] - 1] + Velo_gt[i][j] * Velo_gt[i][j];
        ...
        // cout << "(" << i+1 << "/" << mpMap->vnRMLabel[i][j] << ")" << " s: " << speed_error << " est: " <<
        // sp_est_norm << " gt: " << Velo_gt[i][j] << endl;
  ...
  cout << "average speed error (All Objects):"
       << " s: " << s_sum << "km/h "
       << "Track Num: " << (int)obj_count << " GT AVG SPEED: " << s_gt_sum << endl;
  ...
  for (int i = 0; i < each_obj_count.size(); ++i)
  {
    ...
    if (mpMap->nObjTraCount[i] >= 3)
      cout << endl
           << "average error of Object " << i + 1 << " (" << mpMap->nObjTraCount[i] << "/" << mpMap->nObjTraCountGT[i]
           << "/" << mpMap->nObjTraSemLab[i] << "): "
           << " (est) " << each_obj_est[i] << " (gt) " << each_obj_gt[i] << endl;
  }
