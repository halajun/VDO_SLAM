...
void OpenCvDisplay::drawTracklet(const cv::Mat& rgb, cv::Mat& rgb_tracks, const FeatureTrackletMatrix& static_tracks,
                                 const Map* map)
{
    ...
    if (static_tracklets.exists(current_frame, point_id))
    {
      ...
      if (tracklet.isWellTracked())
      {
        ...
        for (StaticTrackletManager::Observation obs : obs_to_add)
        {
          ...
          cv::putText(rgb_tracks, std::to_string(track_id), cv::Point2i(x - 5, y - 5), cv::FONT_HERSHEY_SIMPLEX, 0.3,
                      colour);
        }
