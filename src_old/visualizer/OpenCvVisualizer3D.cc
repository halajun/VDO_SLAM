...
OpenCvVisualizer3D::OpenCvVisualizer3D(DisplayParams::Ptr params_, Map* map_)
  : Display(params_), map(CHECK_NOTNULL(map_))
{
...
void OpenCvVisualizer3D::drawCurrentCameraPose(WidgetsMap* widgets_map)
{
  ...
  cam_widget_ptr = VDO_SLAM::make_unique<cv::viz::WCameraPosition>(K_, 1.0, cv::viz::Color::green());
...
void OpenCvVisualizer3D::drawTrajectory(WidgetsMap* widgets_map)
{
  ...
  (*widgets_map)["Trajectory"] = VDO_SLAM::make_unique<cv::viz::WTrajectory>(trajectory_viz, cv::viz::WTrajectory::PATH,
