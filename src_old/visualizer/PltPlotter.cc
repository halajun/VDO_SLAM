...
void Plotter::drawDynamicSize(const std::map<int, std::vector<std::vector<gtsam::Key>>>& dynmic_motions,
                              const std::map<int, int>& when_added, const std::string& path)
{
  plt::figure(Plotter::figure);
...
void Plotter::PlotMetricError(Map* map, int max_id, const std::string& path)
{
  ...
  LOG(INFO) << "average error (Over All Objects):"
            << " t: " << t_rpe_sum << " R: " << r_rpe_sum;
  ...
  for (int i = 0; i < each_obj_count.size(); ++i)
  {
    ...
    LOG(INFO) << "average error of Object " << i + 1 << ": "
              << " t: " << each_obj_t[i] << " R: " << each_obj_r[i] << " TrackCount: " << each_obj_count[i];
  }
  ...
  for (int i = 0; i < RigMot.size(); ++i)
  {
    ...
    if (RigMot[i].size() > 1)
    {
      for (int j = 1; j < RigMot[i].size(); ++j)
      {
        ...
        // translation error
        float t_rpe_obj_front = std::sqrt(rpe_obj_frontend.at<float>(0, 3) * rpe_obj_frontend.at<float>(0, 3) +
                                          rpe_obj_frontend.at<float>(1, 3) * rpe_obj_frontend.at<float>(1, 3) +
                                          rpe_obj_frontend.at<float>(2, 3) * rpe_obj_frontend.at<float>(2, 3));
        float t_rpe_obj_back = std::sqrt(rpe_obj_backend.at<float>(0, 3) * rpe_obj_backend.at<float>(0, 3) +
                                         rpe_obj_backend.at<float>(1, 3) * rpe_obj_backend.at<float>(1, 3) +
                                         rpe_obj_backend.at<float>(2, 3) * rpe_obj_backend.at<float>(2, 3));
